...
export function ProductsPage({ initial }: { initial: ProductPage }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try { setProducts(await searchProducts(query, status, category)); }
  ...
  return <section>
    ...
    <form className="filter-bar" onSubmit={submit}><label className="search-field"><Search size={17} /><span className="sr-only">Search products</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search Pixhawk, Raspberry Pi…" maxLength={200} /></label><label><span className="sr-only">Availability</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All states</option><option value="IN_STOCK">In stock</option><option value="OUT_OF_STOCK">Out of stock</option><option value="PREORDER">Pre-order</option><option value="UNKNOWN">Unknown</option></select></label><label><span className="sr-only">Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="">All categories</option><option>Flight Controllers</option><option>Companion Computers</option><option>HATs &amp; Carrier Boards</option></select></label><button className="button button--primary" disabled={busy}>{busy ? "Searching…" : "Apply filters"}</button></form>
...
function ProductCard({ product, history }: { product: Product; history: () => void }) {
  return <article className="product-card"><div className="product-card__top"><span>{product.retailer}</span><StatusBadge status={product.status} /></div><h3>{product.name}</h3><p>{product.manufacturer || "Manufacturer unavailable"} · {product.category}</p><strong className="price">{formatPrice(product.price, product.currency)}</strong><div className="product-card__meta"><span><Clock3 size={14} />{formatTime(product.last_checked_at)}</span></div><div className="product-card__actions"><button className="button button--secondary" onClick={history}>View history</button><SourceLink href={product.canonical_url} /></div></article>;
}
